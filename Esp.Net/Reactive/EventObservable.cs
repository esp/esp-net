...
{
    public delegate void ObserverDelegate<in TModel, in TEvent>(TModel model, TEvent e);
    public delegate void ObserverDelegate<in TModel, in TEvent, in TContext>(TModel model, TEvent e, TContext context);

    public interface IEventObservable<out TModel, out TEvent, out TContext>
    {
        IDisposable Observe(ObserverDelegate<TModel, TEvent> onNext);
        IDisposable Observe(ObserverDelegate<TModel, TEvent, TContext> onNext);
    ...
    internal class EventObservable<TModel, TEvent, TContext> : IEventObservable<TModel, TEvent, TContext>
    {
        ...
        public IDisposable Observe(ObserverDelegate<TModel, TEvent> onNext)
        {
            var streamObserver = new EventObserver<TModel, TEvent, TContext>(onNext);
            return Observe(streamObserver);
        }

        public IDisposable Observe(ObserverDelegate<TModel, TEvent, TContext> onNext)
        {
            var streamObserver = new EventObserver<TModel, TEvent, TContext>(onNext);
            return Observe(streamObserver);
        }
